...
const AllJobWithCompany = async ({
  ...
  const res: IBackendRes<any> = await getJobPagination(current as number, pageSize as number, 'all')
  console.log(res)
...
export default function GetPageJob() {
  ...
  const fetchData = async (current: number, pageSize: number) => {
    setLoading(true)
    setIsLoading(true)
    const allJobWithCompany = await AllJobWithCompany({ current, pageSize, type: 'all' })
